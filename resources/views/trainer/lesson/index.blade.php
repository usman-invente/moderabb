...
<h3 class="page-title float-left d-inline">Lessons</h3>
    ...
    <a href="{{ route('tadd_lessons') }}" class="btn btn-success">Add new</a>
        ...
        <div class="table-responsive">
            <div id="myTable_wrapper" class="dataTables_wrapper no-footer">
                ...
                <tr class="row-{{$val->id}}" id="row-{{$val->id}}">
                    ...
                    <td>
                        <a href="{{route('tedit_lessons',$val->id)}}" class="btn btn-info mb-1" ><i class="fas fa-edit"></i></a>
                        <a class="btn btn-danger mb-1" href="javascript:void(0);"data-id="{{ $val->id }}" data-action="{{ route('tdestroy_lessons',$val->id) }}" onclick="deleteConfirmation({{$val->id}})"><i class="fa fa-trash"></i><a>
    ...
    function deleteConfirmation(id) {
        swal({
            ...
            if (e.value === true) {
                ...
                $.ajax({
                    type: 'GET',
                    url: "{{url('/tranier/delete-lessons')}}/" + id,
