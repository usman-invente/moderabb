...
<form method="POST" action="{{route('tupdate_diploma',$data->id)}}" accept-charset="UTF-8" enctype="multipart/form-data">
@csrf
<div class="card">
...
<div class="card-body">
    ...
    <div class="col-10 form-group">
        ...
        <select class="form-control select2 js-example-placeholder-multiple select2bs4" required multiple="multiple" name="courses">
            ...
            <option value="{{ $tran->id }}" @if($data->courses == $tran->id) ? selected : null @endif >{{ $tran->title }}</option>
            @endforeach
        </select>
        </div>
